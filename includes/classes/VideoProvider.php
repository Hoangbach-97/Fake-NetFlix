...
class VideoProvider{
    public static function getUpNext($conn, $currentVideo){
        ...
        AND id!=:videoId AND ((season=:season AND episode>:episode) 
        OR season>:season) ORDER BY season, episode ASC LIMIT 1");
        ...
        if($query->rowCount()==0) {
            $query = $conn->prepare("SELECT * FROM videos WHERE season<=1 AND episode <=1 AND id!=:videoId ORDER BY views DESC LIMIT 1");
            $query->bindValue(":videoId", $currentVideo->getId());
            $query->execute();
